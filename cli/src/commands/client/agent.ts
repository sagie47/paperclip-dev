      ...
      .action(async (opts: BaseClientOptions & Record<string, string | undefined>) => {
        try {
          ...
          const created = await ctx.api.post<Agent>(`/api/companies/${ctx.companyId}/agents`, {
            ...
            budgetMonthlyCents: parseOptionalNumber(opts.budgetMonthlyCents, "budget-monthly-cents"),
            maxConcurrentIssues: parseOptionalNumber(opts.maxConcurrentIssues, "max-concurrent-issues"),
          });
